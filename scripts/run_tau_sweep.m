...
% 시나리오: 핵심 3개만 (무진동, MIL-STD, MIL+MSS Head)
...
%% ===== 모델 로드 =====
mdl_cell = 'ShipBattery_Model';
mdl_module = 'ShipBattery_Module';
load_system(fullfile(basePath, 'models', [mdl_cell '.slx']));
load_system(fullfile(basePath, 'models', [mdl_module '.slx']));

models = {mdl_cell, mdl_module};
model_labels = {'Cell', 'Module_8S24P'};

%% ===== Sweep 루프 =====
total = length(tau_values) * length(models) * length(scenarios_to_run) * length(k_R_values);
results = struct();
run_count = 0;
...
for ti = 1:length(tau_values)
    ...
    for mi = 1:length(models)
        ...
        for si = 1:length(scenarios_to_run)
            ...
            % ISC timeseries 생성: R_ISC(t) = R_final + (R_init-R_final)*exp(-(t-t_onset)/tau)
            R_isc = ones(size(t_sim)) * 1e6;  % ISC 전 = 매우 큰 저항
            idx = t_sim >= t_onset;
            ...
            for ki = 1:length(k_R_values)
                ...
                fprintf('[%d/%d] %s | case%d(%s) | tau=%ds | kR=%.2f mOhm/g ... ', ...
                    run_count, total, model_labels{mi}, sc_idx, sc.case_label, tau, k_R*1e3);
...
%% ===== 저장 =====
outFile = fullfile(dataPath, 'tau_sweep_results.mat');
save(outFile, 'results', 'tau_values', 'k_R_values', 'scenarios_to_run', ...
     'model_labels', 'R_init', 'R_final', 't_onset', '-v7.3');
fprintf('\nSaved: %s\n', outFile);
